...
def main():
    ...
    pred = []
    w = [line.strip().split('\t')[1] for line in file('%s/predictions-graphics-windows.megam' % data_loc).readlines()]
    for val in w:
        pred.append([val])

    w = [line.strip().split('\t')[1] for line in file('%s/predictions-hockey-baseball.megam' % data_loc).readlines()]
    for x in xrange(len(w)):
        pred[x].append(w[x])

    branches = [] # 1 => graphics-windows, 2 => hockey-baseball
    for y in pred:        
    ...
    for i in xrange(len(branches)):
        ...
        if winner == 1:
            ...
            pred = [graphics[i], windows[i]]
            index = int(argmax(pred)) + 1
            out.write('%d\n' % index)
        else:
            # classify between hockey and baseball
            pred = [hockey[i], baseball[i]]
            index = int(argmax(pred)) + 3
            out.write('%d\n' % index)
